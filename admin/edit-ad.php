...
$created_at  = $con->real_escape_string($_POST['created_at']);
// Get status from the select field (should be "Active" or "Expired")
...
SET name = ?, phone = ?, title = ?, description = ?, price = ?, category_id = ?, brand = ?, country = ?, location = ?, image = ?, created_at = ?, expiry_date = ? 
WHERE id = ?";
...
$stmt_update->bind_param("ssssdissssssi", 
$name, 
$phone, 
$title, 
$description, 
...
echo "<script>alert('Ad updated successfully!'); window.location.href='manage-ads.php';</script>";
} else {
...
<div class="container">
<div class="header">
<a href="manage-ads.php"><i class="fas fa-arrow-left"></i></a>
...
<!-- New fields: Created At (date) and Status -->
<div class="form-group">
<label for="created_at">Created Date*</label>
<input type="date" name="created_at" class="form-control widthset" required value="<?= date('Y-m-d', strtotime($ad['created_at'])); ?>">
</div>
<div class="form-group">
    <p style="color: red;">Please! Update Expiry Date To Republish.</p>
<label for="status">Expiry Date*</label>
<input type="date" name="expiry_date" class="form-control widthset" required value="<?= date('Y-m-d', strtotime($ad['expiry_date'])); ?>">
